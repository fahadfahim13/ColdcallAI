...
git checkout -b feat/t01-inworld-stt-provider
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t01-inworld-stt-provider && git merge main
git push -u origin feat/t01-inworld-stt-provider
open PR

---

## Implementation plan
1. core/voice/stt/base.py -- STTProvider ABC with connect(), disconnect(), stream_audio(audio_gen) -> AsyncIterator[PartialTranscript | FinalTranscript]
...
- Inject mock WS via _model injection point; assert PartialTranscript events arrive
...
## Acceptance Criteria
- [ ] STTProvider ABC in core/voice/stt/base.py with stream_audio() abstract method
...
**Task:** T02
**Title:** InWorld TTS Provider Class
**Est:** 3h | **Phase:** Phase 1 — Voice Core

---

**Goal / why:**
Build the TTS (text-to-speech) abstraction layer. TTSProvider base in core/voice/tts/base.py; InWorldTTSProvider in core/voice/tts/inworld.py streams audio chunks from InWorld TTS WebSocket. stop() closes+reconnects WS for clean barge-in state (avoids draining a stale audio buffer on the next utterance).
...
git checkout main && git pull origin main
git checkout -b feat/t02-inworld-tts-provider
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t02-inworld-tts-provider && git merge main
git push -u origin feat/t02-inworld-tts-provider
open PR

---

## Implementation plan
1. core/voice/tts/base.py -- TTSProvider ABC with connect(), disconnect(), synthesize(text, voice_id) -> AsyncIterator[bytes], stop()
...
## How to test
- Run: python -m pytest tests/
- Mock WS; assert synthesize() yields bytes
...
## Acceptance Criteria
- [ ] TTSProvider ABC in core/voice/tts/base.py with synthesize() and stop() abstract methods
...
**Task:** T03
**Title:** Latency Spike Test (GATE)
**Est:** 2h | **Phase:** Phase 1 — Voice Core

---

**Goal / why:**
Gate test: measure real round-trip latency before building the live pipeline. If P50 > 700ms or P95 > 1400ms, the voice stack is too slow and we switch providers. Three modes: llm_tts (LLM TTFT + TTS first chunk), stt (audio->partial), full (end-to-end). Must pass before T05.
...
git checkout main && git pull origin main
git checkout -b feat/t03-latency-spike
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t03-latency-spike && git merge main
git push -u origin feat/t03-latency-spike
open PR

---

## Implementation plan
1. tests/spike/latency_spike.py -- 20-iteration harness; modes: llm, llm_tts, stt, full
...
## How to test
- Run: python -m pytest tests/
- Run: python tests/spike/latency_spike.py --mode llm --iterations 5
...
## Acceptance Criteria
- [ ] tests/spike/latency_spike.py runs without crash in --mode llm
...
**Test plan:**
- [ ] pytest green
- [ ] Spike runs 5 iterations without crash; P50 printed
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T04
**Title:** Voice Variants Setup
**Est:** 1h | **Phase:** Phase 1 — Voice Core

---

**Goal / why:**
Define the 6 voice personas used for A/B testing. VoiceVariant frozen dataclass holds voice_id + display_name + tags. build_registry(settings) loads from INWORLD_VOICE_* env vars; skips empty ones; raises RuntimeError if zero configured. get_variant() and available_ids() are consumed by T23 (orchestrator) and T38 (Thompson sampling).
...
git checkout main && git pull origin main
git checkout -b feat/t04-voice-variants
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t04-voice-variants && git merge main
git push -u origin feat/t04-voice-variants
open PR

---

## Implementation plan
1. core/voice/variants.py -- VoiceVariant dataclass, build_registry(), get_variant(), available_ids()
...
## How to test
- Run: python -m pytest tests/
- Assert empty voice_id is excluded from registry
...
## Acceptance Criteria
- [ ] VoiceVariant dataclass frozen with voice_id + display_name + tags
...
**Task:** T05
**Title:** Real-Time Conversation Loop
**Est:** 3h | **Phase:** Phase 1 — Voice Core

---

**Goal / why:**
The core pipeline that owns the call from opener to hangup. ConversationLoop wires STT -> LLM -> TTS together. SentenceSplitter dispatches sentences to TTS as LLM streams (latency opt: first sentence starts playing before LLM finishes). History window = 8 turns. VADProvider ABC + NullVAD placeholder (Silero wired in T07). No barge-in yet (T06).
...
git checkout main && git pull origin main
git checkout -b feat/t05-conversation-loop
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t05-conversation-loop && git merge main
git push -u origin feat/t05-conversation-loop
open PR

---

## Implementation plan
1. core/pipeline/conversation_loop.py -- ConversationLoop(stt, tts, llm, llm_model, voice_id, memory, vad)
...
## How to test
- Run: python -m pytest tests/
- Mock all three providers; run 2-turn conversation
...
## Acceptance Criteria
- [ ] ConversationLoop accepts stt, tts, llm, memory, vad params
...
git checkout main && git pull origin main
git checkout -b feat/t06-barge-in
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t06-barge-in && git merge main
git push -u origin feat/t06-barge-in
open PR

---

## Implementation plan
1. core/pipeline/conversation_loop.py -- add _monitor_barge_in(audio_in, barge_in_event), barge_in_vad param
...
## How to test
- Run: python -m pytest tests/
- Inject mock VAD that fires on frame 3; assert TTS stops before completion
...
## Acceptance Criteria
- [ ] _monitor_barge_in() runs as concurrent asyncio task during TTS
...
git checkout main && git pull origin main
git checkout -b feat/t07-silero-vad
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t07-silero-vad && git merge main
git push -u origin feat/t07-silero-vad
open PR

---

## Implementation plan
1. core/voice/vad.py -- SileroVAD(VADProvider): torch.hub.load('snakers4/silero-vad'), 20ms frames buffered to 32ms for inference
...
## How to test
- Run: python -m pytest tests/
- Inject mock model; feed silence frames -> assert no END_OF_TURN
...
## Acceptance Criteria
- [ ] SileroVAD state machine: WAITING -> IN_SPEECH -> SILENCE_AFTER_SPEECH -> END_OF_TURN
...
**Test plan:**
- [ ] pytest green
- [ ] 16 VAD unit tests green (87 total)
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
git checkout main && git pull origin main
git checkout -b feat/t08-call-memory
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t08-call-memory && git merge main
git push -u origin feat/t08-call-memory
open PR

---

## Implementation plan
1. core/call/memory.py -- CallMemory(system_prompt, summarize_fn=None): add_turn(), build_context(), token_estimate()
...
## How to test
- Run: python -m pytest tests/
- Assert turn 9 causes turn 1 to be summarized/dropped
...
## Acceptance Criteria
- [ ] add_turn() slides window at 9th turn (drops or summarizes oldest)
...
git checkout main && git pull origin main
git checkout -b feat/t09-sales-script
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t09-sales-script && git merge main
git push -u origin feat/t09-sales-script
open PR

---

## Implementation plan
1. core/llm/prompts.py -- SYSTEM_PROMPT template, LeadContext + ScriptContext dataclasses, 5 industry templates, AIA objection scripts, build_system_prompt()
...
## How to test
- Run: python -m pytest tests/
- Assert build_system_prompt() fills all static slots for each industry
...
## Acceptance Criteria
- [ ] build_system_prompt() fills all static slots for each of 5 industries
...
git checkout main && git pull origin main
git checkout -b feat/t10-graceful-close
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t10-graceful-close && git merge main
git push -u origin feat/t10-graceful-close
open PR

---

## Implementation plan
1. core/call/close.py -- CallOutcome dataclass (6 fields) + GracefulCloseHandler.build_outcome()
...
## How to test
- Run: python -m pytest tests/
- Assert outcome=no_answer when turn_count==0
...
## Acceptance Criteria
- [ ] CallOutcome.outcome = no_answer when turn_count == 0
...
git checkout main && git pull origin main
git checkout -b feat/t11-browser-harness
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t11-browser-harness && git merge main
git push -u origin feat/t11-browser-harness
open PR

---

## Implementation plan
1. main.py -- FastAPI app with lifespan (pre-warm FasterWhisper + SileroVAD), /health, /ws/call handler
...
## How to test
- Run: python -m pytest tests/
- uvicorn main:app --port 8000 -> open http://localhost:8000 -> click Start Call
...
git checkout main && git pull origin main
git checkout -b feat/t12-phase1-gate
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t12-phase1-gate && git merge main
git push -u origin feat/t12-phase1-gate
open PR

---

## Implementation plan
1. tests/e2e/phase1_gate.py -- already built; 10 scenarios, --auto-only flag for CI
...
## How to test
- Run: python -m pytest tests/
- First: curl https://vllm.bizfinder.ai/health must return 200
...
## Acceptance Criteria
- [ ] Qwen at vllm.bizfinder.ai returns 200 (not 502)
...
**Test plan:**
- [ ] pytest green
- [ ] All 10 gate scenarios PASS (zero FAIL, skips only for InWorld creds)
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T13
**Title:** FreePBX ARI + AudioSocket Setup
**Est:** 6h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Low-level telephony layer: AudioSocket TCP server (receives raw audio from Asterisk), ARIClient (manages Asterisk REST Interface WebSocket for call events), ulaw.py (ulaw<->PCM conversion). TCP_NODELAY must be set -- without it audio buffers in 200ms chunks. No ari-py library (abandoned 2019); raw aiohttp WS for ARI.
...
git checkout main && git pull origin main
git checkout -b feat/t13-freepbx-ari
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t13-freepbx-ari && git merge main
git push -u origin feat/t13-freepbx-ari
open PR

---

## Implementation plan
1. core/telephony/audiosocket.py -- AudioSocketServer (TCP, 3-byte header, TCP_NODELAY, write_buffer_limits=0), AudioSocketSession
...
## How to test
- Run: python -m pytest tests/
- Connect mock TCP client; send valid 3-byte header + payload; assert AudioSocketSession.recv() returns payload
...
## Acceptance Criteria
- [ ] AudioSocketServer binds to TCP port and accepts connections
...
**Task:** T14
**Title:** Outbound Originate
**Est:** 2h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Dial a phone number via FreePBX AMI originate. Campaign orchestrator pops a lead from the CallTarget queue, calls ARIClient.originate(), dialplan routes: HUMAN -> AudioSocket port 9092, MACHINE -> 9093. On HUMAN, wire the live ConversationLoop over AudioSocket.
...
git checkout main && git pull origin main
git checkout -b feat/t14-outbound-originate
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t14-outbound-originate && git merge main
git push -u origin feat/t14-outbound-originate
open PR

---

## Implementation plan
1. core/telephony/dialer.py -- Dialer.dial(lead, campaign) -> CallHandle; wraps ARIClient.originate()
...
## How to test
- Run: python -m pytest tests/
- Mock ARIClient.originate(); assert called with correct phone + channel vars
...
## Acceptance Criteria
- [ ] Dialer.dial() calls ARIClient.originate() with correct channel vars
...
**Test plan:**
- [ ] pytest green
- [ ] Mock dial completes; ConversationLoop invoked on HUMAN answer
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Rotate caller IDs to maximize answer rates. Priority: match area code of prospect. Limit 50 calls/number/day. Flag as spam-likely if answer rate < 5% over last 30 days. caller_id_pool table stores numbers + daily_count + answer_rate + flagged.
...
git checkout main && git pull origin main
git checkout -b feat/t15-caller-id-pool
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t15-caller-id-pool && git merge main
git push -u origin feat/t15-caller-id-pool
open PR

---

## Implementation plan
1. core/telephony/caller_id.py -- CallerIDPool.pick(prospect_phone) -> str | None
...
## How to test
- Run: python -m pytest tests/
- Pool with 3 numbers (2 matching area code, 1 flagged); pick() returns a matching non-flagged number
...
## Acceptance Criteria
- [ ] CallerIDPool.pick() returns same-area-code number when available
...
**Test plan:**
- [ ] pytest green
- [ ] CallerIDPool unit tests green; area-code priority and daily cap enforced
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Est:** 1h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Record every outbound call via Asterisk MixMonitor. After hangup, sync the WAV file to MinIO. Trigger post-call processing job (transcript T26, outcome T27) via ARQ on file save.
...
git checkout main && git pull origin main
git checkout -b feat/t16-call-recording
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t16-call-recording && git merge main
git push -u origin feat/t16-call-recording
open PR

---

## Implementation plan
1. Dialplan: MixMonitor(/tmp/recording-${UNIQUEID}.wav,b) before AudioSocket bridge
...
## How to test
- Run: python -m pytest tests/
- Mock ARI StasisEnd event; assert MinIOClient.upload() called with correct path
...
## Acceptance Criteria
- [ ] MixMonitor writes WAV to /tmp/recording-${UNIQUEID}.wav during call
...
**Test plan:**
- [ ] pytest green
- [ ] Mock StasisEnd -> MinIO upload called -> ARQ job queued
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T17
**Title:** AMD Integration + Tuning
**Est:** 2h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Use Asterisk AMD (Answering Machine Detection) to skip voicemails. amd.conf params from plan Section 3.4. NOTSURE -> treat as human (conservative; avoids dropping live calls). Test with 20-call set; target 70-85% accuracy.
...
git checkout main && git pull origin main
git checkout -b feat/t17-amd-integration
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t17-amd-integration && git merge main
git push -u origin feat/t17-amd-integration
open PR

---

## Implementation plan
1. dialplan/coldcall.conf -- AMD() call before bridge; check AMDSTATUS: HUMAN->bridge, MACHINE->hangup, NOTSURE->bridge
...
## How to test
- Run: python -m pytest tests/
- Run amd_accuracy.py against 20 real calls (10 human, 10 voicemail)
...
## Acceptance Criteria
- [ ] amd.conf deployed with params from plan Section 3.4
...
**Test plan:**
- [ ] pytest green
- [ ] AMD accuracy test: >= 70% human correct, >= 70% machine correct on 20-call set
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T18
**Title:** Concurrency Control + Retries
**Est:** 2h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Limit concurrent active calls per campaign to MAX_CONCURRENT_CALLS. Use asyncio.Semaphore in the ARQ worker. Retry failed dials: max 2 attempts, 60min gap, only for busy/no-answer (not declined/DNC). SELECT FOR UPDATE SKIP LOCKED prevents race conditions in multi-worker setups.
...
git checkout main && git pull origin main
git checkout -b feat/t18-concurrency-retries
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t18-concurrency-retries && git merge main
git push -u origin feat/t18-concurrency-retries
open PR

---

## Implementation plan
1. workers/call_worker.py -- acquire Semaphore before dial; release in finally
...
## How to test
- Run: python -m pytest tests/
- Start MAX_CONCURRENT_CALLS+1 mock dials; assert last one waits for semaphore
...
## Acceptance Criteria
- [ ] MAX_CONCURRENT_CALLS+1 dials: last one waits for semaphore
...
**Test plan:**
- [ ] pytest green
- [ ] MAX+1 concurrent calls: last waits. Busy -> retry enqueued at +60min.
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T19
**Title:** Calling Window Enforcement
**Est:** 1h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Never dial outside 8am-9pm local prospect time (FTC safe harbor). Determine prospect's timezone from state field using pytz. DST handled automatically. Calls queued outside the window are held in ARQ with scheduled ETA at next 8am -- never dropped.
...
git checkout main && git pull origin main
git checkout -b feat/t19-calling-window
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t19-calling-window && git merge main
git push -u origin feat/t19-calling-window
open PR

---

## Implementation plan
1. core/compliance/calling_window.py -- is_callable_now(lead) -> bool; next_callable_time(lead) -> datetime
...
## How to test
- Run: python -m pytest tests/
- 9pm EST prospect -> is_callable_now() = False for PST worker at 6pm (still inside PST window)
...
## Acceptance Criteria
- [ ] is_callable_now() returns False for 9pm+ local prospect time
...
**Test plan:**
- [ ] pytest green
- [ ] Calling window unit tests green including DST; out-of-window calls rescheduled
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
FTC/TCPA compliance: never dial a number on the DNC list. DNC check before every dial. process_opt_out() within 2 seconds of verbal opt-out (TCPA 47 CFR 64.1200). DTMF 9 = opt-out. Keyword detection in transcript: 'remove me', 'stop calling', 'DNC'. National registry scrub every 31 days.
...
git checkout main && git pull origin main
git checkout -b feat/t20-dnc-suppression
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t20-dnc-suppression && git merge main
git push -u origin feat/t20-dnc-suppression
open PR

---

## Implementation plan
1. core/compliance/dnc.py -- DNCList.is_suppressed(phone) -> bool; process_opt_out(phone, source)
...
## How to test
- Run: python -m pytest tests/
- Add phone to dnc_list; assert is_suppressed() returns True before dial
...
## Acceptance Criteria
- [ ] is_suppressed() returns True for phone in dnc_list before dial
...
**Test plan:**
- [ ] pytest green
- [ ] DNC check blocks dialing; opt-out keyword triggers process_opt_out() within 2s
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T21
**Title:** Lead Scoring (Qwen Qualification)
**Est:** 2h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Score each lead 0-100 using Qwen before dialing. Runs async on CSV import (T44), not blocking the caller. Output: score 0-100, tier A/B/C, 2-3 talking points. Score stored in lead.qwen_score + lead.tier. Tier A = dial first, C = skip unless campaign is low on leads.
...
git checkout main && git pull origin main
git checkout -b feat/t21-lead-scoring
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t21-lead-scoring && git merge main
git push -u origin feat/t21-lead-scoring
open PR

---

## Implementation plan
1. core/scoring/lead_scorer.py -- LeadScorer.score(lead: Lead) -> LeadScore; SCORE_PROMPT from plan Section 6.3
...
## How to test
- Run: python -m pytest tests/
- Mock Qwen returning {score:85, tier:'A', talking_points:['...']}; assert stored correctly
...
## Acceptance Criteria
- [ ] LeadScorer.score() returns score 0-100, tier A/B/C, talking_points[]
...
**Test plan:**
- [ ] pytest green
- [ ] LeadScorer unit tests green; score + tier stored on lead record
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Est:** 1h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Generate 2-3 industry-specific value propositions for each lead before dialing. These are injected into the sales script system prompt via the {talking_points} slot. Output: list of short sentences the AI will weave into discovery questions.
...
git checkout main && git pull origin main
git checkout -b feat/t22-talking-points
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t22-talking-points && git merge main
git push -u origin feat/t22-talking-points
open PR

---

## Implementation plan
1. core/scoring/talking_points.py -- TalkingPointsGenerator.generate(lead) -> list[str]
...
## How to test
- Run: python -m pytest tests/
- Mock Qwen; assert talking_points has 2-3 items
...
## Acceptance Criteria
- [ ] TalkingPointsGenerator.generate() returns 2-3 items
...
**Test plan:**
- [ ] pytest green
- [ ] Talking points unit tests green; injected into system prompt
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T23
**Title:** Orchestrator (LeadBrief Builder)
**Est:** 3h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
The orchestrator reads a lead's QA report + Thompson Sampling weights (T38) and builds a LeadBrief: which voice to use, which script variant to use, which opener to use. build_lead_brief(lead, campaign) -> LeadBrief. This is the decision point before every dial.
...
git checkout main && git pull origin main
git checkout -b feat/t23-orchestrator
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t23-orchestrator && git merge main
git push -u origin feat/t23-orchestrator
open PR

---

## Implementation plan
1. core/orchestrator/orchestrator.py -- build_lead_brief(lead, campaign) -> LeadBrief
...
## How to test
- Run: python -m pytest tests/
- Assert LeadBrief.voice_variant_id is one of available_ids()
...
## Acceptance Criteria
- [ ] build_lead_brief() returns fully populated LeadBrief
...
**Test plan:**
- [ ] pytest green
- [ ] Orchestrator unit tests green; LeadBrief fully populated
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T24
**Title:** Variant Tagging Per Call
**Est:** 1h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Every call_result must store the exact voice_variant_id + script_variant_id + opener_used so A/B attribution is possible in T39 (correlation analysis). Without this, Thompson sampling has no signal.
...
git checkout main && git pull origin main
git checkout -b feat/t24-variant-tagging
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t24-variant-tagging && git merge main
git push -u origin feat/t24-variant-tagging
open PR

---

## Implementation plan
1. core/call/result.py -- CallResult dataclass: add voice_variant_id, script_variant_id, opener_used fields
...
## How to test
- Run: python -m pytest tests/
- Run mock loop with LeadBrief; assert outcome.voice_variant_id == brief.voice_variant_id
...
## Acceptance Criteria
- [ ] CallOutcome includes voice_variant_id + script_variant_id + opener_used
...
**Test plan:**
- [ ] pytest green
- [ ] Variant IDs present in CallOutcome and written to DB
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T25
**Title:** QA <> Orchestrator Contract
**Est:** 2h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Define the JSONB schema for qa_report and next_batch_recommendations so the QA agent (T34) and orchestrator (T23) can communicate without coupling. Schema: {voice_rankings[], script_rankings[], winning_patterns[], next_batch_recommendations{a_b_weights, preferred_voice, retire_variants[]}}. See plan Section 11.1.
...
git checkout main && git pull origin main
git checkout -b feat/t25-qa-orchestrator-contract
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t25-qa-orchestrator-contract && git merge main
git push -u origin feat/t25-qa-orchestrator-contract
open PR

---

## Implementation plan
1. core/qa/contract.py -- QAReport + NextBatchRecommendations Pydantic v2 models
...
## How to test
- Run: python -m pytest tests/
- Serialize QAReport to JSON; deserialize; assert field equality
...
## Acceptance Criteria
- [ ] QAReport Pydantic model validates correctly from JSON
...
**Test plan:**
- [ ] pytest green
- [ ] Contract Pydantic models validate correctly; orchestrator applies recommendations
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Post-call: transcribe the audio recording to labeled text [AGENT] / [PROSPECT]. Stored in call_result.transcript (text) and embedded to Qdrant for similarity search in future QA batches. Uses FasterWhisper (already installed) with speaker diarization via pyannote or simple energy-based heuristic.
...
git checkout main && git pull origin main
git checkout -b feat/t26-transcript-generation
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t26-transcript-generation && git merge main
git push -u origin feat/t26-transcript-generation
open PR

---

## Implementation plan
1. core/transcription/transcriber.py -- Transcriber.transcribe(audio_path) -> list[TranscriptTurn]
...
## How to test
- Run: python -m pytest tests/
- Feed a 2-channel WAV (agent left, prospect right)
...
## Acceptance Criteria
- [ ] Transcriber produces TranscriptTurn list with AGENT/PROSPECT labels
...
**Test plan:**
- [ ] pytest green
- [ ] Transcriber unit tests green; transcript in DB + Qdrant after mock call
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T27
**Title:** Outcome Classifier (Qwen Judge)
**Est:** 2h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Post-call QA: Qwen reads the transcript and returns structured judgment. JUDGE_PROMPT (plan Section 6.5): classify outcome enum (booked/interested/callback/declined/no_answer), sentiment (positive/neutral/negative), conversion flag, objections_raised[], structured_answers{}. Stored in call_result as JSONB.
...
git checkout main && git pull origin main
git checkout -b feat/t27-outcome-classifier
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t27-outcome-classifier && git merge main
git push -u origin feat/t27-outcome-classifier
open PR

---

## Implementation plan
1. core/qa/outcome_classifier.py -- OutcomeClassifier.classify(transcript: str) -> ClassificationResult
...
## How to test
- Run: python -m pytest tests/
- Mock Qwen returning valid JSON; assert ClassificationResult fields
...
## Acceptance Criteria
- [ ] OutcomeClassifier.classify() returns ClassificationResult with all fields
...
**Test plan:**
- [ ] pytest green
- [ ] OutcomeClassifier unit tests green; all ClassificationResult fields populated
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
From the Qwen judge output (T27), extract specific facts: budget_mentioned (str), timeline (str), decision_maker_name (str), decision_maker_email (str). Store in call_result.structured_answers JSONB. These feed the write-back to ActiveCampaign (T48) and future personalization.
...
git checkout main && git pull origin main
git checkout -b feat/t28-structured-answers
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t28-structured-answers && git merge main
git push -u origin feat/t28-structured-answers
open PR

---

## Implementation plan
1. core/qa/answer_extractor.py -- AnswerExtractor.extract(classification: ClassificationResult) -> StructuredAnswers
...
## How to test
- Run: python -m pytest tests/
- Feed ClassificationResult with structured_answers dict; assert StructuredAnswers fields
...
## Acceptance Criteria
- [ ] AnswerExtractor extracts all 4 fields: budget, timeline, DM name, DM email
...
**Test plan:**
- [ ] pytest green
- [ ] AnswerExtractor unit tests green; structured_answers written to DB
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Log the cost of every call to the cost_ledger table. Fields: cost_minutes (phone time), cost_tokens (Qwen input+output tokens), cost_tts_chars (EdgeTTS/InWorld chars). Check against COST_CAP_DAILY_USD and COST_CAP_MONTHLY_USD after each insert; pause dialing and alert if exceeded.
...
git checkout main && git pull origin main
git checkout -b feat/t29-cost-capture
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t29-cost-capture && git merge main
git push -u origin feat/t29-cost-capture
open PR

---

## Implementation plan
1. core/billing/cost_ledger.py -- CostLedger.record(call_id, minutes, tokens, tts_chars); check_cap()
...
## How to test
- Run: python -m pytest tests/
- Assert total_usd = minutes*rate + tokens*rate + chars*rate
...
## Acceptance Criteria
- [ ] total_usd = minutes*rate + tokens*rate + chars*rate (correct arithmetic)
...
**Test plan:**
- [ ] pytest green
- [ ] CostLedger unit tests green; cap check triggers at correct thresholds
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T30
**Title:** After-Hours Dumb Script
**Est:** 2h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
During Phase 3 after-hours probe: call businesses at off-hours and ask 2 generic questions (Are you open now? / What are your business hours?). No sales pitch. AMD still runs. Max 30 seconds. Feeds probe_result table for business hours intelligence.
...
git checkout main && git pull origin main
git checkout -b feat/t30-after-hours-script
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t30-after-hours-script && git merge main
git push -u origin feat/t30-after-hours-script
open PR

---

## Implementation plan
1. core/llm/prompts.py -- AFTER_HOURS_PROMPT: role=curious caller, 2 questions only, MAX_TURNS=2, no pitch
...
## How to test
- Run: python -m pytest tests/
- Run mock loop with AFTER_HOURS_PROMPT and max_turns=2
...
## Acceptance Criteria
- [ ] AFTER_HOURS_PROMPT contains no sales pitch text
...
**Test plan:**
- [ ] pytest green
- [ ] Mock probe loop exits after 2 turns; call_type=probe in call_result
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T31
**Title:** Response Classifier (After-Hours)
**Est:** 1h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
After an after-hours probe call, classify the response: live / answering-service / voicemail / rings-to-answer / callback-seen. Store in probe_result. This data feeds lead.business_info with actual business hours.
...
git checkout main && git pull origin main
git checkout -b feat/t31-response-classifier
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t31-response-classifier && git merge main
git push -u origin feat/t31-response-classifier
open PR

---

## Implementation plan
1. core/qa/probe_classifier.py -- ProbeClassifier.classify(transcript) -> ProbeResult
...
## How to test
- Run: python -m pytest tests/
- Transcript 'Our hours are 9 to 5' -> probe_type=live, hours_mentioned='9 to 5'
...
## Acceptance Criteria
- [ ] 'Our hours are 9 to 5' -> probe_type=live, hours_mentioned='9 to 5'
...
**Test plan:**
- [ ] pytest green
- [ ] Probe classifier unit tests green; probe_type correct for each test transcript
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T32
**Title:** Probe Dataset Schema + Export
**Est:** 1h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Store after-hours probe results and export as CSV. probe_result table (Section 9.1). Export probe CSV for analysis. Feed actual business hours back to lead.business_info JSONB.
...
git checkout main && git pull origin main
git checkout -b feat/t32-probe-dataset
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t32-probe-dataset && git merge main
git push -u origin feat/t32-probe-dataset
open PR

---

## Implementation plan
1. alembic/versions/0003_probe_result.py -- CREATE TABLE probe_result (id, lead_id FK, call_id FK, probe_type, hours_mentioned, confidence, called_at)
...
## How to test
- Run: python -m pytest tests/
- Insert 5 probe results; GET /probes/export; assert CSV has 5 rows and correct headers
...
## Acceptance Criteria
- [ ] probe_result migration applies (alembic upgrade head exits 0)
...
**Test plan:**
- [ ] pytest green
- [ ] probe_result migration applies; CSV export correct; business_info updated
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T33
**Title:** After-Hours Stats Report
**Est:** 1h | **Phase:** Phase 3 — PSTN Calling

---

**Goal / why:**
Dashboard card showing after-hours probe statistics: N businesses called, X% answered, peak hours Y-Z. Useful for scheduling sales calls at the right time.
...
git checkout main && git pull origin main
git checkout -b feat/t33-after-hours-stats
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t33-after-hours-stats && git merge main
git push -u origin feat/t33-after-hours-stats
open PR

---

## Implementation plan
1. api/routers/analytics.py -- GET /analytics/probe-stats?campaign_id=X
...
## How to test
- Run: python -m pytest tests/
- Seed 10 probe_results (3 live, 4 voicemail, 3 answering_service)
...
## Acceptance Criteria
- [ ] GET /analytics/probe-stats returns correct counts for seeded data
...
**Test plan:**
- [ ] pytest green
- [ ] probe-stats endpoint returns correct counts from seeded probe_result data
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T34
**Title:** Connect Voice QA Agent
**Est:** 3h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Wire the QA agent into the post-batch pipeline. QA agent reads a batch of transcripts + outcomes, produces a QAReport (T25 schema). POST /qa/run-batch triggers it; ARQ job processes async. QA runs automatically after every 100-call batch (T36).
...
git checkout main && git pull origin main
git checkout -b feat/t34-qa-agent
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t34-qa-agent && git merge main
git push -u origin feat/t34-qa-agent
open PR

---

## Implementation plan
1. core/qa/qa_agent.py -- QAAgent.run_batch(batch_id) -> QAReport; calls Qwen with batch transcripts
...
## How to test
- Run: python -m pytest tests/
- Feed 10 mock transcripts; run QAAgent.run_batch(); assert QAReport.voice_rankings populated
...
## Acceptance Criteria
- [ ] QAAgent.run_batch() produces QAReport with voice_rankings populated
...
**Test plan:**
- [ ] pytest green
- [ ] QAAgent produces valid QAReport from mock transcripts
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T35
**Title:** Voice Performance Ranking
**Est:** 1h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
QA agent ranks voices by segment conversion rate. voice_rankings[] in QAReport: [{voice_id, conversion_rate, sample_count, rank}]. Orchestrator reads rankings to bias Thompson sampling toward top-ranked voices.
...
git checkout main && git pull origin main
git checkout -b feat/t35-voice-ranking
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t35-voice-ranking && git merge main
git push -u origin feat/t35-voice-ranking
open PR

---

## Implementation plan
1. core/qa/voice_ranker.py -- VoiceRanker.rank(call_results: list[CallResult]) -> list[VoiceRanking]
...
## How to test
- Run: python -m pytest tests/
- Voice A: 8/20 conversions; Voice B: 3/20; assert A ranked above B
...
## Acceptance Criteria
- [ ] Voice with 8/20 conversions ranked above voice with 3/20
...
**Test plan:**
- [ ] pytest green
- [ ] VoiceRanker unit tests green; top-converting voice ranked first
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T36
**Title:** Batch Runner
**Est:** 2h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Auto-trigger QA when a batch hits 100 calls. Batch status lifecycle: collecting -> qa_running -> complete. Manual trigger button in dashboard. batch table stores status + call_count + qa_report_id.
...
git checkout main && git pull origin main
git checkout -b feat/t36-batch-runner
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t36-batch-runner && git merge main
git push -u origin feat/t36-batch-runner
open PR

---

## Implementation plan
1. core/batches/batch_runner.py -- BatchRunner.on_call_complete(call_id): increment batch.call_count; if call_count >= 100 -> trigger QA
...
## How to test
- Run: python -m pytest tests/
- Add 99 calls; assert status=collecting
...
**Test plan:**
- [ ] pytest green
- [ ] BatchRunner unit tests green; QA triggers exactly at call 100
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T37
**Title:** Aggregation (Per-Batch Rollup)
**Est:** 2h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
After each batch, compute per-variable breakdown stats for the ANALYSIS_FEATURES matrix (plan Section 9.4). Feature matrix: voice_id x script_id x industry x time_of_day x day_of_week -> conversion rate. Stored in batch.feature_matrix JSONB.
...
git checkout main && git pull origin main
git checkout -b feat/t37-aggregation
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t37-aggregation && git merge main
git push -u origin feat/t37-aggregation
open PR

---

## Implementation plan
1. core/analytics/aggregator.py -- Aggregator.compute_feature_matrix(batch_id) -> FeatureMatrix
...
## How to test
- Run: python -m pytest tests/
- 50 calls with varying voice_id/industry; compute matrix; assert each (voice, industry) cell has rate
...
## Acceptance Criteria
- [ ] feature_matrix has non-empty cells for all ANALYSIS_FEATURES combinations
...
**Test plan:**
- [ ] pytest green
- [ ] Aggregator unit tests green; feature matrix populated for all ANALYSIS_FEATURES
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T38
**Title:** Bayesian Models + Thompson Sampling
**Est:** 3h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Multi-armed bandit for variant selection. BayesianVariantModel: Beta-Binomial per variant (alpha=successes+1, beta=failures+1). ThompsonSamplingAllocator.sample() draws from each variant's beta distribution, returns highest draw. Updated on every call completion (not batch) for low-latency signal. See plan Section 10.2.
...
git checkout main && git pull origin main
git checkout -b feat/t38-bayesian-thompson
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t38-bayesian-thompson && git merge main
git push -u origin feat/t38-bayesian-thompson
open PR

---

## Implementation plan
1. core/ml/bayesian.py -- BayesianVariantModel(variant_id, alpha=1, beta=1): update(success: bool), sample() -> float
...
## How to test
- Run: python -m pytest tests/
- Variant A: alpha=20, beta=5 (80% conversion); Variant B: alpha=5, beta=20 (20%)
...
## Acceptance Criteria
- [ ] Variant A (alpha=20, beta=5) selected > 60 times in 100 ThompsonSampling draws
...
**Test plan:**
- [ ] pytest green
- [ ] Thompson sampling simulation: better variant wins >60% of 100 draws
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Find which features correlate most with conversion. Pearson correlation for numeric features (time_of_day, lead_score), Cramet's V for categorical (industry, voice_id, day_of_week). Output: ranked feature importance list stored in qa_report.feature_importance[].
...
git checkout main && git pull origin main
git checkout -b feat/t39-correlation
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t39-correlation && git merge main
git push -u origin feat/t39-correlation
open PR

---

## Implementation plan
1. core/analytics/correlation.py -- CorrelationAnalyzer.analyze(feature_matrix) -> list[FeatureImportance]
...
## How to test
- Run: python -m pytest tests/
- Synthetic: industry=tech -> 80% conversion, industry=retail -> 20%; assert tech correlation > retail
...
## Acceptance Criteria
- [ ] industry=tech ranks highest in synthetic test (conversion 2x retail)
...
**Test plan:**
- [ ] pytest green
- [ ] tech industry ranks highest in synthetic test with p<0.05
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T40
**Title:** Promote/Demote Logic
**Est:** 2h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Automatically promote winning variants and demote losing ones based on Bayesian evidence. Promote if P(win > baseline) > 80% with >=30 calls. Demote if mean < 50% of best variant with >=30 calls. Never auto-retire -- human approval required (T42).
...
git checkout main && git pull origin main
git checkout -b feat/t40-promote-demote
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t40-promote-demote && git merge main
git push -u origin feat/t40-promote-demote
open PR

---

## Implementation plan
1. core/ml/variant_manager.py -- VariantManager.run_promote_demote(batch_id)
...
## How to test
- Run: python -m pytest tests/
- Variant A: alpha=25,beta=5 vs baseline alpha=10,beta=10; assert A promoted
...
## Acceptance Criteria
- [ ] Variant with P(win > baseline) > 80% promoted (status=promoted)
...
**Test plan:**
- [ ] pytest green
- [ ] Promote triggers with P>0.80; demote triggers with mean<50% of best
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T41
**Title:** Variant Generator (Qwen Proposes New Openers)
**Est:** 2h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
After each QA batch, Qwen analyzes the top 5 transcripts by conversion and proposes 3 new opener variants. Stored as script_variant with status=pending_review and created_by='qwen'. Max 3 proposals per batch -- prevents flooding the review queue.
...
git checkout main && git pull origin main
git checkout -b feat/t41-variant-generator
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t41-variant-generator && git merge main
git push -u origin feat/t41-variant-generator
open PR

---

## Implementation plan
1. core/qa/variant_generator.py -- VariantGenerator.generate(qa_report) -> list[ScriptVariant]
...
## How to test
- Run: python -m pytest tests/
- Mock Qwen returning 3 openers; assert 3 script_variants inserted with status=pending_review
...
## Acceptance Criteria
- [ ] VariantGenerator creates exactly 3 script_variants per batch
...
**Test plan:**
- [ ] pytest green
- [ ] VariantGenerator creates exactly 3 pending_review variants per batch
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Dashboard page for reviewing Qwen-proposed variants before they go live. Approve/reject/modify each proposal. Email alert to fahadfahim13@gmail.com when new proposals arrive. Human MUST approve -- no variant can go from pending_review to active without explicit approval.
...
git checkout main && git pull origin main
git checkout -b feat/t42-human-review
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t42-human-review && git merge main
git push -u origin feat/t42-human-review
open PR

---

## Implementation plan
1. api/routers/variants.py -- GET /variants/pending, POST /variants/{id}/approve, POST /variants/{id}/reject
...
## How to test
- Run: python -m pytest tests/
- POST /variants/{id}/approve; assert status=active in DB
...
## Acceptance Criteria
- [ ] POST /variants/{id}/approve sets status=active in DB
...
**Test plan:**
- [ ] pytest green
- [ ] Approve sets status=active; email fires on new pending_review (mock SMTP)
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Test plan:**
- [ ] pytest green
- [ ] alembic upgrade head exits 0; all tables present in psql \dt
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Bulk-import leads from a CSV file into the lead table. Parse header, map columns to lead schema, normalize phone to E.164, dedup on phone number, bulk INSERT, then async-embed to Qdrant. Runs as ARQ background job triggered by the Campaign UI (T51) file upload.
...
git checkout main && git pull origin main
git checkout -b feat/t44-csv-import
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t44-csv-import && git merge main
git push -u origin feat/t44-csv-import
open PR

---

## Implementation plan
1. core/leads/csv_importer.py -- CSVImporter.import_file(path, campaign_id) -> ImportResult
...
## How to test
- Run: python -m pytest tests/
- Feed test CSV with 10 rows (2 dupes, 1 bad phone); assert 7 inserted, 2 skipped, 1 rejected
...
## Acceptance Criteria
- [ ] 7/10 test rows inserted (2 dupes skipped, 1 bad phone rejected)
...
**Test plan:**
- [ ] pytest green
- [ ] CSVImporter unit tests green; 7/10 test rows inserted, ARQ jobs queued
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T45
**Title:** Phone Normalization Utility
**Est:** 1h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Normalize any phone string to E.164 format (+1XXXXXXXXXX for US) or return None if invalid. Also lookup line type (mobile/landline/voip) via Twilio Lookup API for TCPA compliance (mobile requires written consent).
...
git checkout main && git pull origin main
git checkout -b feat/t45-phone-normalization
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t45-phone-normalization && git merge main
git push -u origin feat/t45-phone-normalization
open PR

---

## Implementation plan
1. core/leads/phone.py -- normalize_phone(raw: str) -> str | None using phonenumbers library
...
**Test plan:**
- [ ] pytest green
- [ ] Phone unit tests green; all US format variants normalize correctly
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T46
**Title:** ActiveCampaign API Integration
**Est:** 3h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Pull contacts from ActiveCampaign filtered by tags/lists and sync to local lead table. Sync runs every 6h via ARQ cron. crm_source='activecampaign', crm_id=AC contact ID stored on lead for write-back (T48).
...
git checkout main && git pull origin main
git checkout -b feat/t46-activecampaign
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t46-activecampaign && git merge main
git push -u origin feat/t46-activecampaign
open PR

---

## Implementation plan
1. core/crm/activecampaign.py -- ACClient(api_key, base_url): list_contacts(tag=None, list_id=None), get_contact(id)
...
## How to test
- Run: python -m pytest tests/
- Mock AC API returning 5 contacts; run sync; assert 5 leads upserted with crm_source='activecampaign'
...
## Acceptance Criteria
- [ ] 5 mock contacts upserted with crm_source='activecampaign'
...
**Test plan:**
- [ ] pytest green
- [ ] Mock AC sync: 5 contacts upserted, duplicate phone updates existing lead
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T47
**Title:** Business Info Replication
**Est:** 1h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Copy lead data from ActiveCampaign to local DB for QA context. Store in lead.business_info JSONB: employee_count, annual_revenue, industry_vertical, tools_used, pain_points from AC custom fields. Used by QA agent (T34) for richer transcript analysis.
...
git checkout main && git pull origin main
git checkout -b feat/t47-business-info
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t47-business-info && git merge main
git push -u origin feat/t47-business-info
open PR

---

## Implementation plan
1. core/crm/activecampaign.py -- ACClient.get_contact_fields(contact_id) -> dict (custom fields)
...
## How to test
- Run: python -m pytest tests/
- Mock AC contact with custom fields; assert lead.business_info contains all mapped fields
...
## Acceptance Criteria
- [ ] lead.business_info populated from AC custom fields after sync
...
**Test plan:**
- [ ] pytest green
- [ ] business_info populated from AC custom fields; missing fields default to None
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T48
**Title:** ActiveCampaign Write-Back
**Est:** 2h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
After call outcome is scored, update the AC contact with last_call_outcome and booked_date (if booked). Runs as ARQ job triggered by post_call_job (T16) completion. Keeps AC CRM in sync with actual call results.
...
git checkout main && git pull origin main
git checkout -b feat/t48-ac-writeback
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t48-ac-writeback && git merge main
git push -u origin feat/t48-ac-writeback
open PR

---

## Implementation plan
1. workers/crm_writeback.py -- write_back_to_ac(call_result_id): load call_result; PATCH AC contact
...
## How to test
- Run: python -m pytest tests/
- booked call result -> assert AC PATCH called with booked_date and last_call_outcome='booked'
...
## Acceptance Criteria
- [ ] booked call result: AC PATCH includes booked_date field
...
**Test plan:**
- [ ] pytest green
- [ ] Mock AC PATCH: booked includes booked_date; declined does not
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T49
**Title:** JSONB Curiosity Fields on Lead
**Est:** 1h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Add a flexible curiosity_fields JSONB column to the lead table. Agents write new facts here as they learn them during calls (e.g., employee count, pain points, current tools). No schema migration per new fact -- just add a key to the JSONB dict.
...
git checkout main && git pull origin main
git checkout -b feat/t49-curiosity-fields
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t49-curiosity-fields && git merge main
git push -u origin feat/t49-curiosity-fields
open PR

---

## Implementation plan
1. alembic/versions/0002_add_curiosity_fields.py -- ALTER TABLE lead ADD COLUMN curiosity_fields JSONB DEFAULT '{}'
...
## How to test
- Run: python -m pytest tests/
- INSERT lead; UPDATE curiosity_fields with {employee_count: 50}; SELECT and assert value
...
## Acceptance Criteria
- [ ] curiosity_fields migration applies (alembic upgrade head exits 0)
...
**Test plan:**
- [ ] pytest green
- [ ] curiosity_fields migration applies; key/value round-trips DB correctly
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T50
**Title:** PostgreSQL + Qdrant Connection Setup
**Est:** 1h | **Phase:** Phase 2 — Orchestrator

---

**Goal / why:**
Initialize async DB connections before any data tasks. asyncpg via PgBouncer (transaction mode). Qdrant client for vector search. Create Qdrant collections: leads_embeddings, call_transcripts, script_variants. Connection pooling config: min_size=5, max_size=20.
...
git checkout main && git pull origin main
git checkout -b feat/t50-db-connections
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t50-db-connections && git merge main
git push -u origin feat/t50-db-connections
open PR

---

## Implementation plan
1. core/db/postgres.py -- get_pool(): asyncpg.create_pool(DATABASE_URL via PgBouncer); lifespan wires pool to app.state
...
## How to test
- Run: python -m pytest tests/
- Start server; hit /health; assert pool is initialized (check app.state.db_pool)
...
## Acceptance Criteria
- [ ] asyncpg pool created on startup (check app.state.db_pool)
...
**Test plan:**
- [ ] pytest green
- [ ] DB pool and Qdrant client initialized at startup; /health returns ok
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Dashboard for creating and managing campaigns. Pages: campaign list, create/edit campaign (name, industry, script_variant, calling_window, concurrency), CSV upload with column-mapping preview, Start/Pause/Stop controls. Uses server-sent events (SSE) for live status.
...
git checkout main && git pull origin main
git checkout -b feat/t51-campaign-ui
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t51-campaign-ui && git merge main
git push -u origin feat/t51-campaign-ui
open PR

---

## Implementation plan
1. api/routers/campaigns.py -- GET /campaigns, POST /campaigns, GET /campaigns/{id}, PATCH /campaigns/{id}/status
...
## How to test
- Run: python -m pytest tests/
- Create campaign via POST; assert 201 with campaign_id
...
## Acceptance Criteria
- [ ] POST /campaigns returns 201 with campaign_id
...
**Test plan:**
- [ ] pytest green
- [ ] Campaign create + CSV upload + status change all work end-to-end
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Real-time dashboard showing active calls, outcome counters, and running cost during a campaign batch. SSE stream from GET /monitor/stream. Kill switch to pause all active calls immediately.
...
git checkout main && git pull origin main
git checkout -b feat/t52-live-monitor
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t52-live-monitor && git merge main
git push -u origin feat/t52-live-monitor
open PR

---

## Implementation plan
1. api/routers/monitor.py -- GET /monitor/stream (SSE): emit call_started, call_ended, cost_update events every 2s
...
## How to test
- Run: python -m pytest tests/
- Start a mock campaign; hit /monitor/stream; assert events within 3s
...
## Acceptance Criteria
- [ ] GET /monitor/stream emits SSE events within 3s of campaign start
...
**Test plan:**
- [ ] pytest green
- [ ] SSE stream emits events; kill switch pauses campaign
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
Post-batch analytics dashboard. Charts: outcome breakdown by voice/script/industry/geo/time, conversion funnel (connected->live->interested->booked), cost per outcome, voice performance leaderboard. CSV export for all views.
...
git checkout main && git pull origin main
git checkout -b feat/t53-results-analytics
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t53-results-analytics && git merge main
git push -u origin feat/t53-results-analytics
open PR

---

## Implementation plan
1. api/routers/analytics.py -- GET /analytics/outcomes, /analytics/funnel, /analytics/cost, /analytics/voice-performance
...
## How to test
- Run: python -m pytest tests/
- Seed 20 mock call_results; hit /analytics/outcomes; assert counts match
...
## Acceptance Criteria
- [ ] /analytics/outcomes returns correct aggregation for 20 seeded call_results
...
**Test plan:**
- [ ] pytest green
- [ ] Analytics endpoints return correct aggregation; CSV export is valid
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T54
**Title:** Loop View Dashboard
**Est:** 3h | **Phase:** Phase 4 — Self-Improving

---

**Goal / why:**
Dashboard showing the self-improving loop progress across batches. Batch 1->N timeline with conversion rate per batch. Variant lineage tree (which Qwen-proposed variants were approved and what they replaced). Voice leaderboard with delta from previous batch. 'What changed' summary card per batch.
...
git checkout main && git pull origin main
git checkout -b feat/t54-loop-view
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t54-loop-view && git merge main
git push -u origin feat/t54-loop-view
open PR

---

## Implementation plan
1. api/routers/loop.py -- GET /loop/batches (timeline), GET /loop/variant-lineage, GET /loop/voice-leaderboard
...
## How to test
- Run: python -m pytest tests/
- Seed 3 batches with different conversion rates; GET /loop/batches; assert 3-point timeline
...
## Acceptance Criteria
- [ ] GET /loop/batches returns 3-point timeline for 3 seeded batches
...
**Test plan:**
- [ ] pytest green
- [ ] Loop view endpoints return correct data for 3 seeded batches
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Goal / why:**
UI to review call recordings with synchronized transcript. Call detail page shows audio player + transcript side-by-side. Click a transcript line to jump to that timestamp. Flag call for QA review. Audio served via MinIO presigned URL (expires 1h).
...
git checkout main && git pull origin main
git checkout -b feat/t55-recording-playback
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t55-recording-playback && git merge main
git push -u origin feat/t55-recording-playback
open PR

---

## Implementation plan
1. api/routers/calls.py -- GET /calls/{id} returns call_result + presigned MinIO URL for audio
...
## How to test
- Run: python -m pytest tests/
- GET /calls/{id}; assert audio_url is a valid URL (mock MinIO)
...
## Acceptance Criteria
- [ ] GET /calls/{id} returns presigned MinIO URL for audio
...
**Test plan:**
- [ ] pytest green
- [ ] Call detail endpoint returns presigned URL; flag PATCH updates DB
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T56
**Title:** Coolify Service Deploy
**Est:** 2h | **Phase:** Deploy + Ops

---

**Goal / why:**
Deploy all services to Coolify via Docker Compose. Services: api (FastAPI), worker (ARQ), postgres (+ PgBouncer), redis, qdrant, minio. See plan Section 18.1 for full docker-compose.yml. Each service has a health check and restart policy.
...
git checkout main && git pull origin main
git checkout -b feat/t56-coolify-deploy
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t56-coolify-deploy && git merge main
git push -u origin feat/t56-coolify-deploy
open PR

---

## Implementation plan
1. docker-compose.yml -- all 7 services from plan Section 18.1; network: coldcall_net
...
## How to test
- Run: python -m pytest tests/
- docker-compose up -d; wait 30s; docker-compose ps -- assert all 7 services Up
...
## Acceptance Criteria
- [ ] docker-compose up -d -> all 7 services Up after 30s
- [ ] curl http://localhost:8000/health returns {status: ok}
...
**Test plan:**
- [ ] pytest green
- [ ] docker-compose ps: all 7 services Up; /health returns ok
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Est:** 1h | **Phase:** Deploy + Ops

---

**Goal / why:**
Centralize all credentials via .env.example + Coolify environment variables. Never commit credentials. One .env per environment (local/.env, staging/.env.staging). .env.example documents every required variable with description and example format.
...
git checkout main && git pull origin main
git checkout -b feat/t57-secrets
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t57-secrets && git merge main
git push -u origin feat/t57-secrets
open PR

---

## Implementation plan
1. .env.example -- all vars from plan Section 18.3: INWORLD_*, QWEN_*, TRELLO_*, DB_*, REDIS_*, MINIO_*, COST_CAP_*
...
## How to test
- Run: python -m pytest tests/
- Unset QWEN_VLLM_ENDPOINT; start server; assert startup fails with clear error
...
## Acceptance Criteria
- [ ] Missing required var fails startup with clear ValidationError
...
**Test plan:**
- [ ] pytest green
- [ ] Missing required var fails startup with clear error; check_env.py exits 0 when all set
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T58
**Title:** Cost Monitoring + Caps
**Est:** 1h | **Phase:** Deploy + Ops

---

**Goal / why:**
Hard caps on daily and monthly spend. COST_CAP_DAILY_USD=100, COST_CAP_MONTHLY_USD=2000. Alert email to fahadfahim13@gmail.com at 80% of either cap. Pause all dialing automatically when cap is hit (POST /monitor/kill internally). Dashboard card shows today's spend vs cap.
...
git checkout main && git pull origin main
git checkout -b feat/t58-cost-monitoring
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t58-cost-monitoring && git merge main
git push -u origin feat/t58-cost-monitoring
open PR

---

## Implementation plan
1. core/billing/cost_monitor.py -- CostMonitor.check_and_alert(): query cost_ledger; compare to caps; send alert if >= 80%
...
## How to test
- Run: python -m pytest tests/
- Seed cost_ledger with $80 today; assert alert email sent to fahadfahim13@gmail.com
...
## Acceptance Criteria
- [ ] 80% of daily cap triggers alert email to fahadfahim13@gmail.com
...
**Test plan:**
- [ ] pytest green
- [ ] 80% triggers email; 100% pauses campaigns (mock SMTP + mock cost_ledger)
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
...
**Task:** T59
**Title:** Full UI Design System (All Pages)
**Est:** 8h | **Phase:** Phase 1+2 — Cross-Phase

---

**Goal / why:**
Replace every placeholder page with a high-professional dark-themed UI. Build a shared design system (coldcall.css + ui.js) once, then apply it to all 7 pages. Animations are ColdCallAI-themed: neural-grid background, waveform bars, phone ripples, AI state badges. Goal: any client demo or test session looks like a real AI SaaS product.
...
git checkout main && git pull origin main
git checkout -b feat/t59-ui-design-system
implement; add/update tests
python -m pytest tests/
git checkout main && git pull origin main
git checkout feat/t59-ui-design-system && git merge main
git push -u origin feat/t59-ui-design-system
open PR

---

## Implementation plan
1. static/css/coldcall.css -- design-system tokens, typography, dark cards, glassmorphism helpers, state-badge CSS, waveform bars, button/pill/form/table styles
...
## How to test
- Run: python -m pytest tests/
- Open each page in browser; verify dark background (#080C14), cyan accent, Inter font
...
## Acceptance Criteria
- [ ] static/css/coldcall.css and static/js/ui.js exist and load without errors
...
**Test plan:**
- [ ] pytest green
- [ ] All 7 pages open with dark theme, animated background, and correct state-specific animations
**Scope check:**
- [ ] Branched off main, merged main back before pushing
- [ ] Tests added for changed behavior
- [ ] No credentials committed
